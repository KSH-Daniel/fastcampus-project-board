...
class PaginationServiceTest {
    ...
    @MethodSource
    @ParameterizedTest(name = "[{index}] 현재 페이지: {0}, 총 페이지: {1} => {2}") // 값을 연속적으로 주입하여, 동일한 대상 메소드를 여러번 출력하는 테스트 기법
    ...
    // MethodSource 에 들어갈 메소드는 테스트 메소드와 이름이 같아야하고, static 선언이 되어야한다, 물론 다르게 가능하지 굳이,,,
    static Stream<Arguments> givenCurrentPageNumberAndTotalPages_whenCalculating_thenReturnsPaginationBarNumbers() {
        return Stream.of(
                arguments(0, 13, List.of(0, 1, 2, 3, 4)), // 1번 체이지, 총 13개 페이지, 표시될 페이지 번호 리스트
